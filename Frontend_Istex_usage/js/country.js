APP.modules.countrys = (function() {
    return {
        ...
        searchcountry: function(query, improve) {
            $.post("/Backend_Istex_usage/src/index.php/getcountrys", {
                ...
                function(data) {
                    if (data == '"Error"') {
                        ...
                        $('.country').append('<div class="ui red inverted center aligned segment" id="nodatacountry"><h4><i class="warningcountry sign icon"></i> No data available!</h4></div>')
                    } else {
                        ...
                        for (var k in parsed['documents']) { // on parcourt le JSON
                            ...
                            if (x < 20) {
                                ...
                                if (k == "") {} else {
                                    ...
                                    circle.bindTooltip("<b>" + occurence + "</b>", {
                                        noHide: true,
                                        permanent: true,
                                        direction: 'center'
                                    }).openTooltip();
                        ...
                        $('.print').on('click', function() { //print de la map
                            $.print("#map", {
                                title: "Map of publications per country for query : " + query
                            });
                        });
                        $('#improve').show();
                        $('#legend h5').remove();
