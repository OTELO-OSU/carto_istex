...
class RequestController 
{
	...
	function Request_alldoc_querypagebypage($query){
		...
		$url='https://api.istex.fr/document/?q='.$query.'&size=*&defaultOperator=AND&output=id,author.affiliations,author.name&facet=genre,corpusName,publicationDate,copyrightDate,language,wos,score';
		...
		$response=self::Curlrequest($url,$curlopt);
			 ...
			 if ($responseencoded->total>=5000) {  // Si les resultats de la requete son superieur a 5000
				...
				$url='https://api.istex.fr/document/?q='.$query.'&size=5000&from=5000&defaultOperator=AND&output=id,author.affiliations,author.name&facet=genre,corpusName,publicationDate,copyrightDate,language,wos,score';
				$curlopt=array(CURLOPT_RETURNTRANSFER => true,
			 	 CURLOPT_ENCODING => "",
			  	CURLOPT_MAXREDIRS => 10,
			  	CURLOPT_TIMEOUT => 40,
				...
				$url='https://api.istex.fr/document/?q='.$query.'&size=5000&from=10000&defaultOperator=AND&output=id,author.affiliations,author.name&facet=genre,corpusName,publicationDate,copyrightDate,language,wos,score';
				$curlopt=array(CURLOPT_RETURNTRANSFER => true,
			 	 CURLOPT_ENCODING => "",
			  	CURLOPT_MAXREDIRS => 10,
			  	CURLOPT_TIMEOUT => 40,
